// Desafio: Sistema de sorteio com avaliação de participantes

/*
Deve sortear aleatoriamente um nome da lista de participantes.
Deve Exibir esse nome com um pequeno atraso (como se fosse um suspense).
Deve avaliar se o participante foi premiado ou não com base na pontuação:
...
Desenvolver função para sortear um nome aleatoriamente.
Desenvolver função para exibir o nome sorteado com 2 segundos de atraso
Desenvolver função para avaliar a pontuação.
Desenvolver função final que organize o fluxo completo do sorteio.
*/

const sortearParticipante = (listaNome) => {
    const indice = Math.floor(Math.random() * listaNome.length);
    return listaNome[indice];
};
